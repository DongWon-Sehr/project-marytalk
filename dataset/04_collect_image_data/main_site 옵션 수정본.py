...
class AutoCrawler:
    def __init__(self, skip_already_exist=True, n_threads=5, do_google=True, do_naver=True, download_path='download',
    ...
    def download_images(self, keyword, links, site_name):
        self.make_dir('{}/{}'.format(self.download_path, keyword.replace('site:','').replace('https://','')))
        total = len(links)
        ...
        for index, link in enumerate(links):
            try:
                ...
                no_ext_path = '{}/{}/{}_{}_{}'.format(self.download_path, keyword.replace('site:','').replace('https://',''), site_name, keyword.replace('site:','').replace('https://',''), str(index).zfill(4))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--threads', type=int, default=8, help='Number of threads to download.')
